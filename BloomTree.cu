...
__device__ bool check_traversal_up_no_mask(int prev, int lca, int src, int dest, int n, uint64_t *hash_value, bool *bit, int h, int m);
__device__ bool check_traversal_down_no_mask(int prev, int lca, int src, int dest, int n, uint64_t *hash_value, bool *bit, int h, int m);
...
__global__ void check_edge(int u, int v, bool *set, uint64_t *hash_value, bool *bit, int h, int m, int n, long int ful_vertices)
{
	int blockNum = blockIdx.z*(gridDim.x*gridDim.y)+blockIdx.y*gridDim.x+blockIdx.x;
	int threadNum = threadIdx.z*(blockDim.x*blockDim.y)+threadIdx.y*blockDim.x+threadIdx.x;
	int tid = blockNum*(blockDim.x*blockDim.y*blockDim.z)+threadNum;
	
	*(set+(tid)*sizeof(bool)) = 0;
	...
	if (v < n && !(u==v) ){
		...
		int lca = calculate_lca(u, v);
		if (check_traversal_up_no_mask(u, lca, src, dest, n, hash_value, bit, h, m)) 
		{
			if(check_traversal_down_no_mask(v, lca, dest, src, n, hash_value, bit, h, m))
			{
				*(set+(tid)*sizeof(bool)) = 1;
			}
		}
	}
}
...
__device__ bool check_traversal_up_no_mask(int prev, int lca, int src, int dest, int n, uint64_t *hash_value, bool *bit, int h, int m)  
{
	int cur = Parent(prev);
	while (cur != lca){
		if (!(CheckBloom((cur*n+src-n+1) << 1, (hash_value)+2*tid*sizeof(uint64_t), bit, h, m)))  return false;
		prev = cur;
		cur = Parent(cur);
	}
	if (!(CheckBloom(((cur*n+src-n+1) << 1) + 1, (hash_value)+2*tid*sizeof(uint64_t), bit, h, m))) return false;
	return true;	
}
...
__device__ bool check_traversal_down_no_mask(int prev, int lca, int src, int dest, int n, uint64_t *hash_value, bool *bit, int h, int m ) 
{
	...
	while (cur != lca){
		if (prev == LeftChild(cur))
		{
			if (!(CheckBloom((cur*n+dest-n+1) << 1, (hash_value)+2*tid*sizeof(uint64_t), bit, h, m))) 
				return false;
		}
		else
		{
			if (!(CheckBloom(((cur*n+dest-n+1) << 1) + 1, (hash_value)+2*tid*sizeof(uint64_t), bit, h, m)))
				return false;
		}
		prev = cur;
		cur = Parent(cur);
...
int * GetNeighbours(int u, bool *h_bits, int num_vertices)
{
	cudaError_t err = cudaSuccess;
...
bool IsEdge(int u, int v, bool *h_bits, int num_vertices, int num_hashes, int num_bits)
{
	int num_ful_levels = floor( log2((double) (2*num_vertices - 1)));
	long int ful_vertices = pow((int) 2,(int) num_ful_levels) - 1;
	...
	bool *d_set = NULL;
	size_t size_set = sizeof(bool);
	cudaMalloc((void **)&d_set, size_set);
	bool *h_set = (bool *)malloc(size_set);

	check_edge<<<1,1>>>(u, v, d_set, d_hash_value, h_bits, num_hashes, num_bits, num_vertices, ful_vertices);
	cudaMemcpy(h_set, d_set, size_set, cudaMemcpyDeviceToHost);
	
	if (h_set == 1) return true;
	else return false;
}
...
int main ()
{
	...
	printf("Get neighbours of?\n");
	scanf("%d",&u);
	
	neighbours = GetNeighbours(u, h_bits, num_vertices);
	
	int a,b;
	
	printf("Enter nodes to check if edge is present between them:\n");
	scanf("%d",&a);
	scanf("%d",&b);
	
	if (IsEdge(a, b, h_bits, num_vertices))
		printf("Edge is present.\n");
	else
